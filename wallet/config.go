...
const (
	...
	unusableFlags = hcashjson.UFWebsocketOnly | hcashjson.UFNotification
)

var (
	hcashdHomeDir            = hcashutil.AppDataDir("/root/.hcashd", false)
	hcashctlHomeDir          = hcashutil.AppDataDir("/hcashdtestcase/wallet", false)
	hcashwalletHomeDir       = hcashutil.AppDataDir("/root/.hcashwallet", false)
	defaultConfigFile      = filepath.Join(hcashctlHomeDir, "hcashctl.conf")
	defaultRPCServer       = "localhost"
	defaultWalletRPCServer = "localhost"
	defaultRPCCertFile     = filepath.Join(hcashdHomeDir, "rpc.cert")
	defaultWalletCertFile  = filepath.Join(hcashwalletHomeDir, "rpc.cert")
)
...
func listCommands() {
	...
	cmdMethods := hcashjson.RegisteredCmdMethods()
	...
	for _, method := range cmdMethods {
		...
		usage, err := hcashjson.MethodUsageText(method)
		if err != nil {
			// This should never happen since the method was just
			// returned from the package, but be safe.
			continue
		...
		if flags&hcashjson.UFWalletOnly != 0 {
...
// config defines the configuration options for hcashctl.
//
...
func cleanAndExpandPath(path string) string {
	...
	if strings.HasPrefix(path, "~") {
		homeDir := filepath.Dir(hcashctlHomeDir)
...
// For this it tries to read the hcashd config file at its default path, and extract
// the RPC user and password from it.
func createDefaultConfigFile(destinationPath string) error {
	// Nothing to do when there is no existing hcashd conf file at the default
	// path to extract the details from.
	hcashdConfigPath := filepath.Join(hcashdHomeDir, "hcashd.conf")
	if !fileExists(hcashdConfigPath) {
		return nil
	}

	// Read hcashd.conf from its default path
	hcashdConfigFile, err := os.Open(hcashdConfigPath)
	if err != nil {
		return err
	}
	defer hcashdConfigFile.Close()
	content, err := ioutil.ReadAll(hcashdConfigFile)
	if err != nil {
		return err
	}
