...
const (
	...
	defaultLogFilename         = "hcashwallet.log"
	...
	defaultMaxFee                    hcashutil.Amount = 1e6
	defaultMinFee                    hcashutil.Amount = 1e5
...
type config struct {
	...
	Username               string                  `short:"u" long:"username" description:"Username for legacy JSON-RPC and hcashd authentication (if hcashdusername is unset)"`
	Password               string                  `short:"P" long:"password" default-mask:"-" description:"Password for legacy JSON-RPC and hcashd authentication (if hcashdpassword is unset)"`
...
//
// The above results in hcashwallet functioning properly without any config
...
func loadConfig() (*config, []string, error) {
	...
	if cfg.DisableClientTLS {
		...
		// If CAFile is unset, choose either the copy or local hcashd cert.
		if !cfg.CAFile.ExplicitlySet() {
			cfg.CAFile.Value = filepath.Join(cfg.AppDataDir.Value, defaultCAFilename)

			// If the CA copy does not exist, check if we're connecting to
			// a local hcashd and switch to its RPC cert if it exists.
			...
			if !certExists {
				if _, ok := localhostListeners[RPCHost]; ok {
					hcashdCertExists, err := cfgutil.FileExists(
						hcashdDefaultCAFile)
					if err != nil {
						fmt.Fprintln(os.Stderr, err)
						return loadConfigError(err)
					...
					if hcashdCertExists {
						cfg.CAFile.Value = hcashdDefaultCAFile
					}
				}
			}
		}
	...
	// If the hcashd username or password are unset, use the same auth as for
